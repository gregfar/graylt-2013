...
refseq.tss <- read.table(args[1],col.names=bed.names,colClasses=bed.classes)
xpb <- read.table(args[2],col.names=bed.names,colClasses=bed.classes,skip=1)
xpd <- read.table(args[3],col.names=bed.names,colClasses=bed.classes,skip=1)
orb <- read.table(args[4],header=T,sep="\t",
...
ggplot(data=u.tss.xpb, aes(x=distance)) +
  ...
  theme_pub()
ggsave(paste(results,"/Figure_S1aL.png",sep=""),width=3,height=3)

ggplot(data=u.tss.xpd, aes(x=distance)) +
  geom_histogram(aes(y = (..count..)/14570),binwidth=50,fill="darkgreen") +
  scale_x_continuous(limits=c(-1000,1000)) + ylim(0,.018) +
  theme_pub()
ggsave(paste(results,"/Figure_S1aR.png",sep=""),width=3,height=3)
...
# output the plot
ggsave(paste(results,"/Figure_3b.tiff",sep=""),height=3,width=3)
 
  
# output a summary table
write.table(binding,paste(results,"/tss_binding_summary.txt",sep=""),sep="\t",row.names=F,quote=F)